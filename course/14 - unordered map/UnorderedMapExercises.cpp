...
StringIntMap makeWordCounts(const StringVec& words) {
  ...
  for (unsigned long i = 0; i < words.size(); i++) {
      string key = words[i];
      int count = wordcount_map[key];
      if (count > 0) {
          wordcount_map.erase(key);
      }
      wordcount_map[key] = count + 1;
...
int lookupWithFallback(const StringIntMap& wordcount_map, const std::string& key, int fallbackVal) {
  ...
  // =================================================
  int found = wordcount_map.count(key);
  if (!found) { return fallbackVal; }
  return wordcount_map.at(key); // Change this!
}
