...
static void treeFactory(GenericTree<int>& tree) {
  ...
  //using TreeNode = typename GenericTree<int>::TreeNode;

  auto rootPtrNode = tree.getRootPtr();
  rootPtrNode -> data = 4;
  auto nodeA = rootPtrNode->addChild(8);
  auto nodeB = nodeA->addChild(16);
  nodeB->addChild(42);
  nodeA->addChild(23);
  rootPtrNode->addChild(15);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  // root
  auto toVisit = rootNodePtr -> childrenPtrs;
  results.push_back(rootNodePtr->data);
  while (!toVisit.empty()) {
      vector<TreeNode*> nextLevel;
      unsigned long visitSize = toVisit.size();
      for (unsigned long i = 0; i < visitSize; i++) {
          results.push_back(toVisit[i] -> data);
          unsigned long childNodesSize = toVisit[i] -> childrenPtrs.size();
          for (unsigned long j = 0; j < childNodesSize; j++) {
              nextLevel.push_back(toVisit[i] -> childrenPtrs[j]);
          }
      }
      toVisit = nextLevel;
  }
